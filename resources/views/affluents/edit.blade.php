...
<div class="max-w-2xl mx-auto py-10 px-6">
    <h2 class="text-2xl font-bold text-gray-800 mb-6 flex items-center gap-2">
        ...
        Modifier l'affluent : {{ $affluent['nomAffluent'] ?? '' }}
    </h2>

    <form method="POST" action="{{ route('affluents.update', rawurlencode($affluent['nomAffluent'])) }}"
        ...
        <div>
            ...
            <input type="number" step="0.1" name="longueurAffluent"
                   ...
                   value="{{ old('longueurAffluent', $affluent['longueurAffluent'] ?? '') }}" required>
        ...
        <div>
            ...
            <input type="number" step="0.1" name="debitAffluent"
                   class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                   value="{{ old('debitAffluent', $affluent['debitAffluent'] ?? '') }}" required>
            @error('debitAffluent')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
